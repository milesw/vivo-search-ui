...
4. Change the Solr URL and VIVO URL in the search prototype configuration. Get the Solr URL from Miles. The instance URL should be the URL for your VIVO test instance.

   > vi productMods/js/search/prototype_files/vivoSearchPrototype.config.js
   [Look for the 'solrUrl' and 'prototypeURL' variables]

5. Move to the Vitro source tree and patch the search controller to use only a single Freemarker template.

   > cd /usr/local/vitro-trunk/
   > patch -p0 < /usr/local/vivo-trunk/search_prototype--vitro_search_controller.patch

6. Move back to VIVO source tree and re-deploy. You will need to restart Tomcat for the controller changes to take effect.
